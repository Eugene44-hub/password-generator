...
class Generator {
    ...
    generatePassword() {
        ...
        let lower = this.lowerBox;
        let upper = this.lowerBox;
        let number = this.lowerBox;
        let symbol = this.lowerBox;

        let generatedPassword = '';

        if (countBox === 0)
            return '';
        let typeVal = [{ lower }, { upper }, { number }, { symbol }].filter(item => Object.values(item))
